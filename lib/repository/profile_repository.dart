...
class ProfileRepository {
  Future<Result?> getTermsPrivacy(BuildContext context,
    ...
    appNetwork.getNetworkResponse(requestModal, context, (result, isSuccess) {
      if (isSuccess) {
        ...
        Map<String, dynamic> map = (result as SuccessState).value as Map<
            String,
            dynamic>;
        if (map['data'] is List<dynamic>) {
          ...
          CacheDataManager.cacheData(key: StringConstant.kPrivacyTerms,
              jsonData: map,
              isCacheRemove: true);
  ...
  Future<Result?> uploadImage(List<String> imagePath, BuildContext buildContext,
    ...
    appNetwork.getNetworkResponse(
        requestModal, buildContext, (result, isSuccess) {
      if (isSuccess) {
        var response = ASResponseModal.fromResult(result);
  ...
  Future<Result?> getUserProfileDetails(BuildContext buildContext,
    ...
    appNetwork.getNetworkResponse(requestModal, buildContext,
          ...
          if (isSuccess) {
            ...
            if (map['data'] is Map<String, dynamic>) {
              response.dataModal = UserInfoModel.fromJson(map['data']);
              CacheDataManager.cacheData(key: StringConstant.kUserDetails,
                  jsonData: map,
                  isCacheRemove: true);
            }
            networkResponseHandler(Result.success(response), isSuccess);
  ...
  Future<Result?> deleteUserAccount(BuildContext context,
      NetworkResponseHandler responseHandler) async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    AppNetwork appNetwork = AppNetwork();
    var header = {
  ...
  Future<dynamic> uploadProfile(BuildContext context, FileReader reader,
      File file, ApiCallback responseData) async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    var url = '${NetworkConstants.kAppBaseUrl +
        NetworkConstants.kUploadProfilePic}';
    var request = http.MultipartRequest('POST', Uri.parse(url));
    request.headers['Authorization'] =
        "Basic " + sharedPreferences.get("token").toString();
    ...
    reader.onLoadEnd.listen((e) {
      ...
      request.send().then((response) async {
        if (response.statusCode == 200) {
          String responseBody = await response.stream.transform(utf8.decoder)
              .join();
